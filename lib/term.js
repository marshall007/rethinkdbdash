...
Term.prototype.changes = function(options) {
    var self = this;
    self._arityRange(arguments, 0, 1, "changes", self); 
    var term = new Term(self._r);
...
Term.prototype.map = function() {
    this._arityRange(arguments, 1, Infinity, "map", this); 
...
Term.prototype.createWriteStream = function(options) {
    var self = this;

    if (self._query[self._query.length-2] !== termTypes.TABLE) {
        throw new Error.ReqlDriverError("`createWriteStream` is not defined", query._query);
    }

    var WriteStream = require(__dirname+"/write-stream");
    return new WriteStream(self, options);
}
